...
const services = [
  ...
  },
  {
    ...
    slug: "commercial-electrical-solutions",
  },
  {
  ...
  },
  {
    ...
    slug: "residential-commercial-generator-installation",
  },
];
...
const Navbar: React.FC = () => {
  ...
  return (
    <>
      ...
      <nav className="absolute top-0 left-0 right-0 bg-black shadow-lg z-[100] transform-gpu will-change-transform border-b-2 border-red-600">
        ...
        <div className="w-full flex justify-between items-center h-48 lg:h-44">
          ...
          <div className="hidden lg:flex items-center space-x-12 xl:space-x-16 2xl:space-x-20 ml-auto pr-8">
            <Link
              href="/#"
              className="text-red-600 hover:text-red-400 text-3xl font-normal transition-colors duration-200"
            >
              About Us
            </Link>
            <div className="relative" ref={dropdownRef}>
              ...
              {isServicesDropdownOpen && (
                <div
                  className="absolute left-1/2 transform -translate-x-1/2 top-full mt-2 bg-black border border-gray-800 rounded-md shadow-lg z-[110] min-w-max"
                  onMouseEnter={() => setIsServicesDropdownOpen(true)}
                ...
                >
                  <div className="py-2">
                    {services.map((service, index) => (
                      <button
                        key={service.slug}
                        className={`block w-full text-left px-6 py-3 text-xl text-red-600 hover:bg-red-600 hover:text-white transition-colors font-bold whitespace-nowrap ${
                          index !== services.length - 1 ? 'border-b border-gray-800' : ''
                        }`}
                        onClick={() =>
                          handleServiceJump(normalizeSlug(service.title))
                        }
                        type="button"
                      >
                        {service.title}
          ...
          <div className="hidden lg:block absolute bottom-4 left-1/2 transform -translate-x-1/2">
            <a
              href="tel:+1234567890"
              className="text-red-600 hover:text-red-400 text-xl font-medium transition-colors duration-200"
            >
              📞 (631) 764-1577
            </a>
          </div>
        ...
        <div
          className={`lg:hidden transition-all duration-300 ease-in-out relative z-[110] ${
            isMenuOpen
              ? "max-h-screen opacity-100"
              : "max-h-0 opacity-0 overflow-hidden"
          }`}
        >
          <div className="bg-black border-t border-red-600 overflow-y-auto max-h-[calc(100vh-12rem)]">
                ...
                {isMobileServicesDropdownOpen && (
                  <div className="mt-4 bg-gray-900 border border-gray-800 rounded-md shadow-lg">
                    {services.map((service) => (
                      <button
                        key={service.slug}
                        className="block w-full text-center px-4 py-4 text-lg text-red-600 hover:bg-red-600 hover:text-white transition-colors font-normal border-b border-gray-800 last:border-b-0"
                        onClick={() =>
                          handleServiceJump(normalizeSlug(service.title))
                        }
                        type="button"
